    ...
    <>
      ...
      {questions.length === 0 ? (
        <div className="mt-10 flex flex-col items-center justify-center rounded-[24px] border border-slate-200 bg-slate-50 px-8 py-20 text-center">
          ...
          <p className="mt-5 text-xl font-semibold text-slate-900">No published questions found</p>
          <p className="mt-2 text-sm leading-7 text-slate-500">
            Questions will appear here once they are published in the database.
          </p>
        </div>
      ) : (
        <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {questions.map((q) => (
            <div
              key={q.id}
              className="flex flex-col rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md"
            >
              <div className="flex items-start justify-between gap-3">
                <h2 className="text-base font-semibold text-slate-950">{q.title}</h2>
                <span
              ...
              <p className="mt-1 text-xs font-medium text-slate-500">{q.topic}</p>

              {q.short_summary && (
                <p className="mt-3 text-sm leading-6 text-slate-600">{q.short_summary}</p>
              )}
